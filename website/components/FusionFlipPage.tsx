...
function FusionCard({ row }: { row: FusionFlipRow }) {
  ...
  return (
    <div className="flip-card" style={{ position: 'relative' }}>
      <div style={{ position: 'absolute', inset: '0 0 auto 0', height: 3, background: '#7c5cbf' }} />
      ...
      <div style={{ padding: '14px 14px 10px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            <div style={{ fontSize: '0.72rem', color: '#7c5cbf', marginTop: 2, fontWeight: 600 }}>
              {row.steps}-step fusion · saves {savingsPct}% vs direct buy
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: '12px 14px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        <div>
          <div style={{ fontSize: '0.68rem', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 700 }}>Fusion Cost</div>
          <div style={{ color: '#ef4444', fontWeight: 800 }}>{coins(row.fusionCost)}</div>
          <div style={{ fontSize: '0.65rem', color: 'var(--muted)', marginTop: 1 }}>vs {coins(row.rawCost)} direct</div>
        </div>
        <div>
        ...
        <div>
          <div style={{ fontSize: '0.68rem', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 700 }}>Fusions (10M)</div>
      ...
      <div style={{ padding: '0 14px 10px' }}>
        ...
        <div style={{ marginTop: 6, fontSize: '0.75rem', color: 'var(--muted)' }}>
          {coins(row.profitPerFusion)}/fusion · Fill {row.fillScore} · Vol {coins(row.weeklyVolume)}/wk
        </div>
      </div>
      ...
      {row.chain.length > 0 && (
        <div style={{ padding: '0 14px 14px' }}>
          <div style={{ fontSize: '0.68rem', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 700, marginBottom: 6 }}>Craft chain</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
            {row.chain.map((step, i) => (
              <span key={i} style={{
                background: 'rgba(124,92,191,0.15)', border: '1px solid rgba(124,92,191,0.3)',
                borderRadius: 4, padding: '2px 7px', fontSize: '0.72rem', color: '#9d7fe8', fontWeight: 600,
              }}>{step}</span>
            ))}
          </div>
        </div>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        <aside style={{ width: 220, background: 'var(--sidebar)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', padding: '20px 12px', gap: 4, flexShrink: 0 }}>
          ...
          <nav style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <Link href="/fusion" className="nav-item active" style={{ textDecoration: 'none' }}>
        ...
        <main style={{ flex: 1, overflow: 'auto', padding: '24px 28px' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', marginBottom: 18 }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                ...
                <span style={{ color: 'var(--muted)', letterSpacing: '0.08em', textTransform: 'uppercase', fontSize: '0.72rem', fontWeight: 800 }}>Fusion page</span>
              </div>
              <h1 style={{ fontSize: '1.9rem', fontWeight: 900, color: '#e6eefb', lineHeight: 1.05 }}>Fusion Flips</h1>
              <p style={{ marginTop: 8, maxWidth: 860, color: 'var(--muted)', lineHeight: 1.6 }}>
                Multi-step compound crafts: instead of buying intermediate ingredients at bazaar price, craft them yourself from cheaper raw materials. Only shows items where the chained craft path is cheaper than direct ingredient purchase — true fusion profit.
              </p>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(120px, 1fr))', gap: 10, width: 'min(100%, 440px)' }}>
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '14px 16px' }}>
                ...
                <div style={{ marginTop: 6, fontSize: '1.15rem', fontWeight: 800, color: '#7c5cbf' }}>{top ? coins(top.totalProfit) : '—'}</div>
              </div>
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '14px 16px' }}>
              ...
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '14px 16px' }}>
                <div style={{ fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--muted)', fontWeight: 800 }}>Fusion items</div>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '18px 20px', marginBottom: 24 }}>
            ...
            <div style={{ color: 'var(--muted)', lineHeight: 1.6 }}>
              The algorithm traces every item&apos;s recipe tree up to 4 levels deep. At each step, it picks the cheaper option: buy from bazaar or craft from sub-ingredients. If the fully-chained cost is lower than buying everything directly, that&apos;s a fusion flip. Sorted by total profit in a 10M budget.
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 16 }}>
            ...
            {!loading && visibleRows.length === 0 && (
              <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '60px 0', color: 'var(--muted)', border: '1px dashed var(--border2)', borderRadius: 14, background: 'rgba(255,255,255,0.02)' }}>
                ...
                <div style={{ fontWeight: 600 }}>No fusion opportunities found</div>
                <div style={{ fontSize: '0.82rem', marginTop: 4, opacity: 0.7 }}>Bazaar prices may be tight. Check back shortly.</div>
              </div>
            )}
